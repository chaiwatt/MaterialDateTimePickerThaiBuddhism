 ...
 * github: miamarti
 * *
 * แก้ไข พ.ศ. chaiwat npcsolution.com
 * */
    ...
    Plugin.prototype = {
        ...
        initDates: function () {
            if (this.$element.val().length > 0) {
                ...
                if (typeof (this.params.format) !== 'undefined' && this.params.format !== null) {
                    ...
                    this.currentDate = moment(elementval).locale(this.params.lang);
        ...
        showDate: function (date) {
            if (date) {
                ...
                this.$dtpElement.find('.dtp-actual-day').html(this.convertDay(date.locale(this.params.lang).format('dddd')));
                this.$dtpElement.find('.dtp-actual-month').html(this.convertMonth(date.locale(this.params.lang).format('MMM').toUpperCase()));
                this.$dtpElement.find('.dtp-actual-num').html(date.locale(this.params.lang).format('DD'));
                this.$dtpElement.find('.dtp-actual-year').html(parseInt(date.locale(this.params.lang).format('YYYY'))+ yearconst);
            }
        },
        ...
        convertFullMonth: function (month)
        {
           if(month == "JAN"){
               return "มกราคม";
            }else if(month == "FEB"){
        ...
        constructHTMLCalendar: function (date, calendar) {
            ...
            _template += '<div class="dtp-picker-month">' + this.convertFullMonth(date.locale(this.params.lang).format('MMM').toUpperCase()) + "  " + (parseInt(date.locale(this.params.lang).format('YYYY'))+ yearconst) + '</div>';
            _template += '<table class="table dtp-picker-days"><thead>';
            for (var i = 0; i < calendar.week.length; i++) {
                // _template += '<th>' + moment(parseInt(calendar.week[i]), "d").locale(this.params.lang).format("ddd").substring(0, 1) + '</th>';
                _template += '<th>' + this.convertShortDay(moment(parseInt(calendar.week[i]), "d").locale(this.params.lang).format("dd")) + '</th>';
            }
        ...
        setElementValue: function () {
            ...
            if(this.params.time === true){
                if(res[0].length <=2){
                    var _str = str.split(" ");
                    var _tmp = _str[0].split("/"); 
                    newdate = _tmp[0] + "/" + _tmp[1] + "/" + (parseInt(_tmp[2])+ yearconst) + ' '  + _str[1]; 
                }else{
                    newdate = res[0];
                }
            }         
        ...
        _onYearItemClick: function (e) {
            ...
            var newYear = $(e.currentTarget).data('year')-yearconst;
